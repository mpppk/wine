...
//
// **web検索での裏取りは両者とも行う**(#474)。#358 では「銘柄数 × 検索でコストが発散する」
// として一括抽出だけ裏取りを外し、裏取りしたい銘柄は登録後に単体のエチケット解析を使う
// 住み分けにしていた。その住み分けは利用者から見ると「解析を2回する」ことに他ならず、
// 完了通知から二度目の登録に進める導線が二重登録を生んだ(#472)。発散させないための
// 歯止めは**銘柄ごとにリクエストを立てない**ことで担保する: 1回の推論(サーバー側ツール
// ループ)の中でまとめて調べさせ、検索回数は上限で打ち切る。
//
