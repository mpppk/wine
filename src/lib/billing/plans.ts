...
//
// 無料枠を 150 → 200 に上げたのは #474。一括抽出が web検索で裏を取るようになり、
// Claude 経路(Sonnet 5)の写真1枚が 126 → 156 クレジットになって 150 に収まらなく
// なったため。**「写真1枚の解析は無料会員の付与内に収まる」を不変条件として維持する**
// 側を選んでいる(wine-list-extraction.test.ts が固定)。見積を実費より薄くして枠に
// 押し込む案は採らない: 予約を超えた消費は settle が頭打ちにするので、差額がそのまま
// 原価の持ち出しになる。

/** 無料会員に毎月付与するクレジット数(= 許容原価 $0.20/月)。無料 < プレミアム。 */
export const MONTHLY_CREDITS_FREE = 200;
