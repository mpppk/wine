...
//
// ═══════════════════════════════════════════════════════════════════════════
// 🚨 **現状このスロットルは実環境で機能していない**(2026-08-03 実測・原因未特定)
//
// デプロイ済みの wine-preview に対し、上限 30/60s の POST /api/upload を同一
// セッションで 45回連続・80並列で投げても **429 が1件も返らなかった**。
// 一方で:
//  - `wrangler versions view` はバインディングが配備済みと表示する
//    (env.RATE_LIMIT_UPLOAD (30 requests/60s) 等)
//  - 全リクエストが同一 colo(IAD)に着弾していて、colo 分散が理由ではない
//  - **同じコードは miniflare(rate-limit.workers.test.ts)では期待どおり false を返す**
//  - env[binding] の動的参照を静的参照に変えても変化なし
//
// つまり limit() が常に success を返している。下の「判定できないときは通す」設計に
// より動作は壊れないが、**#397 の脅威は塞がれていない**。この状態を「対処済み」と
// 読まないこと。原因調査(最小再現 Worker でアカウント/プラン側の問題かを切り分ける)か、
// D1 ベース(#397 の当初案)への切り替えが要る。
// 再検証手順は docs/deployment.md の「レートリミットが効いているかの確認」。
// ═══════════════════════════════════════════════════════════════════════════
//
