...
// シャンパーニュの生産者は awards を持たない(#224)。
//
// **シャンパーニュには生産者の公的格付けが存在しない**。エシェル・デ・クリュ
// (グラン・クリュ17村 / プルミエ・クリュ42村)が格付けするのは村であって造り手ではない
// ため、村の格付けを生産者の受賞として付け替えることはできない。
//
// 選定の一次情報に使った Club Trésors de Champagne(Special Club)の公開会員名簿も
// awards には載せない。ProducerAward は「授与元・受賞名・年」が揃う受賞を表す型だが、
// 会員名簿は年次で入れ替わる在籍リストで、公開されているのは現在の会員25社の顔ぶれだけ
// (各社の加入年は公表されていない)。年を持てない以上、受賞として登録すると
// 「裏が取れたものだけ入れる」規約から外れる。会員であることは選定理由として
// data-integrity テストと docs/architecture.md に残し、UI には出さない。
//
// 年次ガイド(Bettane+Desseauve / La Revue du Vin de France 等)の掲載リストは
// 「リストという成果物」側なので転記しない(ローヌ #225・ボージョレと同じ扱い)。
