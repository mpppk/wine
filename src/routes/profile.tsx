						...
						<div className="flex items-center gap-4">
							...
							<div className="flex flex-col gap-2">
								...
								<p className="text-xs text-muted-foreground">
									{PHOTO_FORMATS_LABEL}、最大{MAX_PHOTO_SIZE_LABEL}
								</p>
							</div>
						</div>
						{selectedFile && (
			...
			<CardContent className="flex flex-col gap-4">
				...
				{billingQuery.isPending || billingQuery.isError ? null : isPremium ? (
					...
					<div className="flex flex-col gap-2">
						...
						<p className="text-xs text-muted-foreground">
							月額{formatJpyAmount(PREMIUM_PRICING.monthlyAmount)}
							円(年払いなら
							{formatJpyAmount(PREMIUM_PRICING.annualAmount)}円で
							{premiumAnnualSavedMonths()}
							ヶ月分お得)。広告非表示などの特典があります。
						</p>
					</div>
				)}
			</CardContent>
