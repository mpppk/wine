...
3. **推論実行** — OpenRouter を呼ぶ（`src/lib/ai/openrouter.ts` が唯一の接続口）。
   `OPENROUTER_API_KEY` 未設定なら予約の前に利用不可として扱い、推論しない。
   失敗時は予約を返却し、他プロバイダへの直接フォールバックはしない（#602）。
4. **確定・差分返却** — 実測 usage を**実際に推論したモデルの単価**で µUSD に換算し、予約との
   差分を `refund` で戻す。推論失敗時は予約全額を `refund` で戻す。

   > **意図した経路ではなく実行した経路の単価で課金する**（降格が無いため両者は一致するが、
   > 単価換算は「実際に推論したモデル」で行う規律は維持する）。換算は `ai-service.chargeFor`
