...
- **「クリマである」と「AOC である」も直交**: 法的アペラシオンかどうかは `isLegalAppellation()` が唯一の権威（モンラシェ=クリマかつ単独 AOC、シャブリのレ・クロ=クリマだが非 AOC）。`kind` から AOC かどうかを推論するコードを書かない。**「AOC/DOC(G) でない」の中身も一様ではない**: IGT は DOC(G) の下位に位置する法的な地理的表示（EU の IGP）なので、バッジ文言は `getNonAppellationBadgeJa()` が制度名（`IGT`）を出す。フランス語制度の否定形「非 AOC」をイタリアのエントリに出さない（#212）。
- **`principal`（主要品種）は「呼称が主要品種を定めている」ことの表明**: 定めのない呼称（IGT）は `principal` を持たない。1 つでも置くと `aop-variety` / `variety` クイズが「この呼称の主要品種は◯◯」と事実でない断定をするため、`grapes` は全て `accessory` にする（テストで IGT だけが例外であることを固定）。
...
- **IGT は「DOC(G) の枠外で造る造り手を事実に即した呼称で載せる」ためだけに持つ**（#212）。`toscana-igt` 1件のみで、Montevertine（1981年にキアンティ・クラッシコを離脱）と Masseto がここに載る。**すでに DOC(G) 側に載っている生産者を IGT にも重複させない**（Antinori・Le Macchiole は IGT の銘柄も持つが、生産者としてはボルゲリ等に載っている）。この絞り込みは意図的なので、網羅されていないと判断して勝手に広げない
