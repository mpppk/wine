...
| AI | OpenRouter（chat completions。地域 Q&A・エチケット解析・ワインリスト解析）。シークレット `OPENROUTER_API_KEY` 設定時のみ実行し、未設定なら予約の前に利用不可として扱う。OpenAI / Anthropic への個別接続と Workers AI バインディングは #602 で廃止。どの経路を走らせるかは `resolveLabelRoute`（`src/lib/ai/config.ts`）が SSOT |
...
- **`idApp` の帯規約**: GeoJSON との結合キー。INAO の実値のほか、実体のないエントリには地域ごとの合成 ID 帯（900001〜シャンパーニュ格付け村、910001〜ボルドー、920001〜/921001〜イタリア、930001〜ブルゴーニュのクリマ・合成総称ノード等。詳細は `types.ts` のコメント）を割り当てる。**`idApp >= 930000`（`POLYGONLESS_IDAPP_MIN`）はポリゴンを持たない帯**で、ジオデータ生成・整合テストの対象外。この定数の**唯一の情報源は `src/lib/wine/geo-bands.json`** で、TS 側は `types.ts` が re-export し、Node ESM のジオデータ生成スクリプトは `scripts/geo-bands.mjs` 経由で同じ JSON を読む（#407）。両者が同じ値を見ていることは `geo-bands.test.ts` が固定する。
...
- **ユーザ単位マスタの重複は DB の unique 制約ではなくサービス層の関門で弾く**。`place`（場所）は同名を作れないが、制約は張らず `prepareNewPlace`（`place-service.ts`）が SELECT → INSERT で判定する。場所を作る全経路（銘柄の新規登録・写真からの一括登録・目撃記録の追加/編集・`createPlace`）が必ずここを通り、同名なら 409 と「一覧から選んでください」という**直し方**を文言で返せる。制約で弾くと文言を選べないうえ、同名が既にある利用者を登録不能にする（既存行のある表への UNIQUE 追加はマイグレーション自体が通らない。横断規約の「マイグレーション検証の限界」も参照）。判定はユーザ単位・名前の完全一致（zod が trim 済み）で、支店は「〇〇 渋谷店」のように名前で区別する。文言は `duplicatePlaceNameMessage`（`lib/place/place.ts`）が単一情報源で、サーバの 409 と入力欄の事前警告が同じ文を使う。**当初は「同名の別支店もある」として重複を許し表記ゆれの抑制を UI（サジェスト）に委ねていたが、目撃記録のたびに場所を作れるようにした時点でその前提が崩れたため方針を反転させた**（#605）。SELECT → INSERT なので同時実行ではすり抜けうるが、その結果は従来どおりの同名 2 件であって壊れた状態ではない。
