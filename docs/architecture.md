...
第2層のガイドは**選定の内部参考にとどめ、アプリには一切出さない**。生産者名そのものは事実で著作権が及ばないが、**どの生産者を選ぶかというガイドの「選択」自体**が創作的表現（著作権法12条の2 のデータベースの著作物）およびEUの sui generis データベース権（Directive 96/9/EC。創作性を要求せず、取得・検証・提示への実質的投資があれば発生し、実質的部分の抽出を禁じる）の対象になりうる。

- ガイドの名称・星の数・掲載事実を**UIに出さない**。`ProducerInfo.sources` にも入れない
- **単一ガイドの丸写しにしない**。第1層・複数の第2層・第3層を突き合わせて自前の基準で選定する
- **解説文は公式サイト等の一次情報から自分で書く**。ガイドの記述を翻訳・要約しない

`ProducerInfo.sources` に置いてよいのは「リンクできる一次情報」だけ — 公的格付けの公式ページ / MICHELIN Grapes のような選出事実を載せた公式記事 / 生産者の公式サイト / 生産者団体の公開会員名簿。判定基準は `producer-info.ts` の JSDoc が単一情報源。
