...
* ローカルのシークレットが必要な場合は `bun run dev:vars` で `.dev.vars` を生成する（1Password の wine vault から `OPENROUTER_API_KEY`・`STRIPE_SECRET_KEY` を展開。vault に無い値は空のまま出るので手動で埋める。OpenAI / Anthropic のキーは使わない。詳細は `.dev.vars.example` 参照）
...
* ブラウザでの動作確認中はスクリーンショットを適宜撮影し、`gh` の `--attach` フラグでPRに直接添付してください（Gyazoは使わない。要 gh v2.99.0+）。
  * 例: 本文ファイルに `![aop map](./aop-map.png)` と書いて `gh pr edit <番号> --body-file body.md --attach ./aop-map.png` を実行すると、ローカルパスがアップロード済みURLに置換される（代替テキスト維持。ただし代替テキスト内の `・` はファイル名に置換されるので避ける）
* 動作確認の完了後は、結果をPRのdescriptionに追記してください。結果には撮影したスクリーンショットの画像を記載してください。

### GitHub MCP はPR本文の一部のマークダウンをサニタイズする

**Claude Code on the web など GitHub MCP 経由でPRを作成・更新すると、本文の一部の記法が落ちる。** 上記2つの規約（detailsタグ・画像添付）がまさに影響を受けるので、書いたつもりでも反映されていないことがある。複数セッションで発生を確認済み（#208 で顕在化）。画像添付は `gh --attach`（CLI直接実行）では起きない。
