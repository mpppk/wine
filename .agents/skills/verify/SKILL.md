...
- このときスクリプトは、**`bun --version` が 1.3.13 以下なら Bun ではなく Node で実行する**(1.3.13 以下の bun は `fetch` の ClientHello に ECH GREASE(`encrypted_client_hello` 拡張)を付け、agentプロキシのTLS終端がそのハンドシェイクを黙って捨てるため "The socket connection was closed unexpectedly"(ECONNRESET) になる)。スクショのPR添付は Gyazo ではなく **`gh` の `--attach` フラグ**を使う（要 gh v2.99.0+。本文ファイルに `![説明](./shot.png)` と書いて `gh pr edit <番号> --body-file body.md --attach ./shot.png` でローカルパスがURLに置換される。代替テキスト内の `・` はファイル名に置換されるので避ける）
...
1. ローカルで同一内容を検証: `bun run db:migrate:local`(PRのマイグレーション込み) → `bun run dev` + 上記Playwright手順でスクショ撮影（PRへの添付は `gh --attach`。要 gh v2.99.0+）
