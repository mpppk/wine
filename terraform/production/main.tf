# 本番環境用の Stripe リソース(ライブモード)。
# 認証: STRIPE_API_KEY にライブモードのシークレットキー(sk_live_...)を設定して実行する。
...
module "stripe" {
  ...
  # 公開カスタムドメイン。webhook・Billing Portal の戻り先の基準。
  app_url = "https://wine.nibo.sh"
}
