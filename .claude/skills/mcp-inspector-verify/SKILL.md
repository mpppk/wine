...
description: todo-app2026のMCPサーバー（src/lib/mcp/ や src/routes/api/mcp.ts）変更時に、MCP InspectorでOAuth接続〜tools/list〜list_todos実行〜（UI変更なら）AppsタブでのApp描画までを実機確認する手順。MCP関連のTest Plan実施時に使う。
---
...
6. （任意・切り分け用）`POST /api/mcp` に `Authorization: Bearer <token>` ＋ `Accept: application/json, text/event-stream` で `initialize` → `tools/list` → `tools/call list_todos` を叩き、`ui://todo-app2026/todos` リソース同梱を先に確認しておく。
...
5. **Tools タブ**: `List Tools` で 7 ツール（get_current_user / list_organizations / list_teams / list_todos / create_todo / update_todo / delete_todo）が出ること。`List todos` を選び team_id を入れ **Run Tool** → **Tool Result: Success**、結果に todo 配列と `ui://todo-app2026/todos`、右パネル Meta に `ui.resourceUri` があることを確認。
